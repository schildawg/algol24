...
⚠️ **A violation does not compile either, and fails differently.** A source file is a translation unit, so a name belonging to another module is `static` to another file and `cc` refuses it — `use of undeclared identifier`. Both halves of the implementation therefore reject the program, at different stages and in different words. What is *not* diagnosed is the language rule itself: neither compiler explains that the name exists and was not imported, which is what the interpreted message above is for.
...
The interpreter defines the language, so anywhere the compiler is **narrower** than the interpreter is part of the observable surface and belongs here. There are eighteen such places in `CEmitter.java` and **seventeen** in `algc/CEmitter.a24`. ⚠️ The odd one is `A class field`: `algc/Expr.a24` has no `ClassVarExpr` node, so there is nothing for algc to refuse and nothing a program could reach. Every other row is in both. Each one **names the construct, writes no output, and stops** — it never emits C that means something other than what the program said, because output comparison is the only thing verifying the back end and silence would defeat it.
...
| `Two modules exporting 'X'` | Reachable. Two imported modules exporting one name. Both interpreters refuse it at the `uses` as `'X' is already defined; mark it private in one of the modules.` ⚠️ Without this row the compilers emitted it: two *functions* then failed at the **linker**, and two *variables* did not fail at all — file-scope tentative definitions merge into one object, so the program linked and quietly used whichever unit initialised last. Mark one `private`, or rename. |
...
| `An identifier containing '<c>'` | A guard on the identifier mangler; `<c>` is the offending character, interpolated. Only `?` is legal in an Algol-24 name and illegal in C, and that is mapped to `_q`; this is the one place that would have to learn about a second such character. |
...
- **Two modules cannot export one name.** Refused at the `uses` as `'X' is already defined; mark it private in one of the modules.`, and by both compilers before they emit — two exported symbols of one name do not link, and for a *variable* they silently merge instead, which is worse. Qualified access does not rescue it: `KA.Same()` and `KB.Same()` both read correctly, but the emitted symbols still collide, so the remedy is `private` or a rename. See [Units](#units-and-qualified-names), and `WISHLIST.md` for what qualifying emitted symbols would cost.
...
- **The type checker does not know about module boundaries.** A reference to another module's private name types cleanly and fails at run time as `Undefined variable`. ⚠️ Not always: where the checker happens to know a type for that name from the file that *does* declare it, the program is rejected earlier and as `Type mismatch!` — the wrong problem, named confidently. Both implementations do it identically. Correct, but later than it could be.
