...
anything that moved. It has no opinion about whether the new behavior is
...
and the conformance case asserts the behavior we want instead — failing until
...
./spec/probes/record.sh --record   # accept the current behavior as recorded
```
...
⚠️ Color is **transliterated**, not stripped — `\033[31m` is recorded as
`[RED]`. Stripping is not injective, so a wrong color and a right one would
compare equal afterwards and a color change would be invisible to the
